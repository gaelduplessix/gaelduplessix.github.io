...
Renderer.prototype.setShaderSource = function (vertex, fragment) {
...
Renderer.prototype.getShader = function(source, type) {
    ...
    // Get number of area lights (needs to be added to number of objects)
    ...
    source = source.replace('{NB_OBJECTS}', this.scene.objects.length + areaLightCount);
    ...
    for (var i = 0; i < this.scene.lights.length; ++i) {
        if (this.scene.lights[i] instanceof AreaLight) {
    		intersectWithObjectsFunction += 'intersectWithObject(ray, objects[' + (this.scene.objects.length + k) + '], kMin, intersectedObject);';
            ++k;
    	}
...
Renderer.prototype.buildShaders = function() {
    ...
    for (var i = 0; i < this.scene.objects.length + this.scene.lights.length; ++i) {
    	this.pathTracerShader.sceneUniforms.objects.push({
	    	...
	    	data: this.gl.getUniformLocation(this.pathTracerShader, 'objects[' + i + '].data'),
            data2: this.gl.getUniformLocation(this.pathTracerShader, 'objects[' + i + '].data2')
    	});
    }
    
    for (var i = 0; i < this.scene.lights.length; ++i) {
    	this.pathTracerShader.sceneUniforms.lights.push({
            type		: this.gl.getUniformLocation(this.pathTracerShader, 'lights[' + i + '].type'),
	    	color		: this.gl.getUniformLocation(this.pathTracerShader, 'lights[' + i + '].color'),
            intensity   : this.gl.getUniformLocation(this.pathTracerShader, 'lights[' + i + '].intensity'),
            v1      	: this.gl.getUniformLocation(this.pathTracerShader, 'lights[' + i + '].v1'),
            v2      	: this.gl.getUniformLocation(this.pathTracerShader, 'lights[' + i + '].v2'),
            v3      	: this.gl.getUniformLocation(this.pathTracerShader, 'lights[' + i + '].v3')
    	});
    }
...
Renderer.prototype.setSceneUniforms = function(program) {
	...
	for (var i = 0; i < this.scene.objects.length; ++i) {
		...
		// Object data depends on its type
		var data = [0, 0, 0], data2 = [0, 0, 0];
		if (this.scene.objects[i] instanceof Sphere) {
			data[0] = this.scene.objects[i].radius;
		} else if (this.scene.objects[i] instanceof Plane) {
			data = this.scene.objects[i].normal;
		} else if (this.scene.objects[i] instanceof Cube) {
            data = this.scene.objects[i].max;
        } else if (this.scene.objects[i] instanceof Parallelogram) {
            data = this.scene.objects[i].a;
            data2 = this.scene.objects[i].b;
        }
		
		this.gl.uniform3fv(program.sceneUniforms.objects[i].data, data);
        this.gl.uniform3fv(program.sceneUniforms.objects[i].data2, data2);
	}

	// Send area lights as objects
    var k = 0;
    for (var i = 0; i < this.scene.lights.length; ++i) {
        if (this.scene.lights[i] instanceof AreaLight) {
	...
	for (var i = 0; i < this.scene.lights.length; ++i) {
        this.gl.uniform1i(program.sceneUniforms.lights[i].type, this.scene.lights[i].LightID);
		this.gl.uniform3fv(program.sceneUniforms.lights[i].color, this.scene.lights[i].color);
        this.gl.uniform1f(program.sceneUniforms.lights[i].intensity, this.scene.lights[i].intensity);

        // Light data depends on its type
        if (this.scene.lights[i] instanceof PointLight) {
            this.gl.uniform3fv(program.sceneUniforms.lights[i].v1, this.scene.lights[i].position);
        } else if (this.scene.lights[i] instanceof AreaLight) {
            this.gl.uniform3fv(program.sceneUniforms.lights[i].v1, this.scene.lights[i].position);
            this.gl.uniform3fv(program.sceneUniforms.lights[i].v3, this.scene.lights[i].a);
            this.gl.uniform3fv(program.sceneUniforms.lights[i].v2, this.scene.lights[i].b);
        }
	}
}